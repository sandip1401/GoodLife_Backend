...
const addDoctor = async (req, res) => {
  try {
    ...
    // get doctor image first
    const imageFile = req.file;

    if (!imageFile) {
      return res.json({
        success: false,
        message: "Doctor image required",
      });
    }
...
const AdmincancelAppointment = async (req, res) => {
  try {
    ...
    appointment.isCompleted = false; // important fix
...
const getAllPatients = async (req, res) => {
  try {
    const patients = await userModel.find().select("name email phone image");
    res.json({
      success: true,
      patients,
    });
